...
import {
    concat, merge, share, mapTo, delay, tap, timeInterval,
    timeout, delayWhen, switchMap, map, switchMapTo, repeat, concatMap, debounceTime, debounce, skip, distinctUntilChanged, mergeMap, pairwise, startWith, throttle, mergeAll, concatAll, combineAll
} from 'rxjs/operators';
...
range(1, 100).pipe(
    concatMap(i => of(i).pipe(delay(200 + (Math.random() * 500)))),
    ...
    pairwise(),
    // tap(console.log),
    // debounce(([p, c]) => !p || !c ? of(1) : ((c['c'] - p['c']) > 500) ? of(1) : interval(1000)),
    map(([_, value]) => ({ ...value, l: Date.now() })),
    map((value) => concat(of(value), of(value).pipe(delay(3000)))),
    switchMapTo(),
).pipe(
